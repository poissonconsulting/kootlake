# Authors and Citation

## Authors

- **Jeff Burrows**. Author, data contributor.

- **Matt Neufeld**. Author, data contributor.

- **Greg Andrusak**. Author, data contributor.
  [](https://orcid.org/0009-0008-0093-8521)

- **Marley Basset**. Author, data contributor.

- **Eva Schindler**. Author, data contributor.

- **[Joe
  Thorley](https://www.poissonconsulting.ca/author/joe-thorley/)**.
  Author, maintainer, data contributor.
  [](https://orcid.org/0000-0002-7683-4592)

- **[Evan
  Amies-Galonski](https://www.poissonconsulting.ca/author/evan-amies-galonski/)**.
  Author. [](https://orcid.org/0000-0003-1096-2089)

- **Rhonda Gates**. Data contributor.

- **Jess Spencer**. Data contributor.

## Citation

Source:
[`DESCRIPTION`](https://github.com/poissonconsulting/kootlake/blob/main/DESCRIPTION)

Burrows J, Neufeld M, Andrusak G, Basset M, Schindler E, Thorley J,
Amies-Galonski E (2026). *kootlake: Kootenay Lake Data*. R package
version 0.3.1.9003, <https://github.com/poissonconsulting/kootlake>.

    @Manual{,
      title = {kootlake: Kootenay Lake Data},
      author = {Jeff Burrows and Matt Neufeld and Greg Andrusak and Marley Basset and Eva Schindler and Joe Thorley and Evan Amies-Galonski},
      year = {2026},
      note = {R package version 0.3.1.9003},
      url = {https://github.com/poissonconsulting/kootlake},
    }
